...
const uniqueKeyProperty = 'uniqueKey';
...
export default function ({ types: t }) {
  ...
  const visitJSXElement = (path, state) => {
    ...
    const getNewUniqueKey = () => String(uniqueKeyCounter ++);
    ...
    const JSXElement = node => jsxObjectTransformer(
      t.objectExpression([
        ...
        t.objectProperty(t.identifier(childrenProperty), node.closingElement ? JSXChildren(node.children) : t.arrayExpression([])),
        t.objectProperty(t.identifier(uniqueKeyProperty), t.stringLiteral(getNewUniqueKey()))
      ])
    );
